...
import { TmSafeAreaView } from "../../Components/TmSafeAreaView"
...
export default ({ navigation, route }) => {
  ...
  const isValidatedClassData = () => {
    if (!classData?.classAvatar?.small) {
      Toast.show('Please uplaed class photo.', Toast.LONG)
      return;
    }
    if (!classData?.className) {
      Toast.show('Please enter class name.', Toast.LONG)
      return;
    }
    if (!classData?.classDescription) {
      Toast.show('Please enter class description.', Toast.LONG)
      return;
    }
    return true
  }
  ...
  const handleSubmitClassForm = () => {
    ...
    Toast.show('Class list updated.', Toast.LONG)
      ...
      [
        ...
        },
        {
          text: "OK", onPress: () => {
            ...
            Toast.show('Class deleted.', Toast.LONG)
          }
        }
      ],
      { cancelable: false }
  ...
  const handleSubmitJoinForm = () => {
    ...
    if (!inviteCodeToJoin) {
      Toast.show('Please enter invite code.', Toast.LONG)
      return;
    }
    handleCloseJoinForm()
  }
